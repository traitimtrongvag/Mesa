...
pub async fn run_server(port: u16) {
    ...
    let ws_route = warp::path("ws")
        ...
        .map(|ws: warp::ws::Ws, clients: Clients| {
            // clone clients để async block không giữ reference không Send
                                                            ...
                                                            if let Some(recipient_aes_key) = recipient.aes_key {
                                                                ...
                                                                if let Ok(encrypted) = recipient_cipher.encrypt(
                                                                    ...
                                                                    let _ = recipient.tx.send(Message::text(
                                                                        format!("[Encrypted]:{}", serde_json::to_string(&encrypted_msg).unwrap())
                                                                    ));
                                                                }
                                                            }
                                                        }
